...
def main() -> None:
    ...
    messages = [LlmMessage(role="user", content_blocks=[TextBlock(type="text", content="說一個笑話，並用工具計算2+2的答案")])]
    resp = provider.call(messages)
    messages.append(resp)
    messages.append(LlmMessage(role="user", content_blocks=[TextBlock(type="text", content="你沒有給我講笑話?只有呼叫工具?")]))
    resp2 = provider.call(messages)
    print(resp)
